<template>
  <div ref="wheel" class="vue-wheel-select">
    <div class="scroller" ref="scroller" @scroll="settleScroll">
      <div
        :style="{
    ...
    <div class="overlay">
      <div :style="{ height: `${optionHeight}px` }" class="center-bounds" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    setValue() {
      ...
      let idx =
        (middleScrollPos - (middleScrollPos % this.optionHeight)) /
        this.optionHeight;
    ...
    scrollToValue(value) {
      ...
      const selectedIdx =
        this.options.indexOf(value) + (this.allowNullSelection ? 1 : 0);
